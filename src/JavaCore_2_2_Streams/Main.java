    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < 10_000_000; i++) {
            persons.add(new Person(
                    ...
                    JavaCore_2_2_Streams.Sex.values()[new Random().nextInt(JavaCore_2_2_Streams.Sex.values().length)],
                    JavaCore_2_2_Streams.Education.values()[new Random().nextInt(JavaCore_2_2_Streams.Education.values().length)])
            );
        }
        ...
        List<String> recruit = persons.stream().filter(person -> (person.getAge() >= 18 && person.getAge() <= 27))
                .filter(person -> person.getSex() == JavaCore_2_2_Streams.Sex.MAN).map(Person::getFamily)
        ...
        Collection<Person> ableBodied = persons.stream().
                filter(person -> person.getEducation() == JavaCore_2_2_Streams.Education.HIGHER)
                .filter(person -> person.getSex() == JavaCore_2_2_Streams.Sex.MAN ? person.getAge() >= 18 && person.getAge() <= 65
